...
When the review context includes `pr-remote` or `branch-remote`, the working tree is **not** the reviewed head. Do **not** use Read/Grep on workspace paths for files in the changed-file list — they may not match the branch or PR under review.

Instead:

- Prefer `git show <remote-head-ref>:<path>` when `<PR_HEAD_REF>` or `<BRANCH_HEAD_REF>` is provided in context.
